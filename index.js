...
const { isUrl, generateMessageTag, getBuffer, getSizeMedia, await, sleep, reSize } = require('./Library/lib/myfunc')
...
conn.sendMessage("6281221380234@s.whatsapp.net", {text: `Kzy Bot Berhasil Terkoneksi!`})
console.log('[Kzy Bot Berhasil Terkoneksi!]' + JSON.stringify(conn.user.id, null, 2));
}
});
...
pathFile = await writeExif(media, { packname: options.packname ? options.packname : 'KZY Bot', author: options.author ? options.author : '+628248845231', categories: options.categories ? options.categories : [] })
await fs.promises.unlink(filename)
await conn.sendMessage(from, {sticker: {url: pathFile}}, {quoted})
